...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const [docentesRes, segMateriasRes, todasClavesRes, firmasRes] = await Promise.all([
      ...
      supabaseAdmin
        .from('seguimiento_materia')
        .select('docente_id, clave_id, asistencia, temarios_oficina, temarios_bs, evaluacion_intermedia, evaluacion_final, publicacion_calificaciones')
        .eq('ciclo', '2027-1'),
      supabaseAdmin
        .from('claves')
        .select('id, docente')
    ...
    const segPorClave: Record<number, any> = {}
    for (const s of segMaterias) {
      segPorClave[s.clave_id] = s
    }
    ...
    const data = docentes.map((d: any) => {
      ...
      if (numMaterias === 0) return { ...d, num_materias: 0, porcentaje: 0 }

      let totalSI = 0
      for (const clave of misClaves) {
        const seg = segPorClave[clave.id]
        const tieneAcuse = firmasSet.has(clave.id)
        for (const campo of CAMPOS_SEG) {
          let valor = seg?.[campo] || null
          if (campo === 'temarios_oficina' && tieneAcuse) valor = 'SI'
          if (valor === 'SI') totalSI++
        }
      }

      const porcentaje = Math.round((totalSI / (numMaterias * CAMPOS_SEG.length)) * 100)
